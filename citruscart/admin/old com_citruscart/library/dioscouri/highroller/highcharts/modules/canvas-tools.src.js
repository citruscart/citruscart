...
function RGBColor(color_string)
{
    ...
    var color_defs = [
        {
            ...
            citruscart: ['rgb(123, 234, 45)', 'rgb(255,234,245)'],
    ...
    this.toHex = function () {
        var r = this.r.__toString(16);
        var g = this.g.__toString(16);
        var b = this.b.__toString(16);
        if (r.length == 1) r = '0' + r;
        if (g.length == 1) g = '0' + g;
        if (b.length == 1) b = '0' + b;
        return '#' + r + g + b;
    ...
    this.getHelpXML = function () {
        ...
        var citruscarts = new Array();
        // add regexps
        for (var i = 0; i < color_defs.length; i++) {
            var citruscart = color_defs[i].citruscart;
            for (var j = 0; j < citruscart.length; j++) {
                citruscarts[citruscarts.length] = citruscart[j];
            }
        }
        // add type-in colors
        for (var sc in simple_colors) {
            citruscarts[citruscarts.length] = sc;
        }

        var xml = document.createElement('ul');
        xml.setAttribute('id', 'rgbcolor-citruscarts');
        for (var i = 0; i < citruscarts.length; i++) {
            try {
                var list_item = document.createElement('li');
                var list_color = new RGBColor(citruscarts[i]);
                var citruscart_div = document.createElement('div');
                citruscart_div.style.cssText =
                        'margin: 3px; '
                        + 'border: 1px solid black; '
                        + 'background:' + list_color.toHex() + '; '
                        + 'color:' + list_color.toHex()
                ;
                citruscart_div.appendChild(document.createTextNode('test'));
                var list_item_value = document.createTextNode(
                    ' ' + citruscarts[i] + ' -> ' + list_color.toRGB() + ' -> ' + list_color.toHex()
                );
                list_item.appendChild(citruscart_div);
		...
		svg.Font = new (function() {
			...
			this.CreateFont = function(fontStyle, fontVariant, fontWeight, fontSize, fontFamily, inherit) { 
				...
				return { 
					...
					__toString: function () { return [this.fontStyle, this.fontVariant, this.fontWeight, this.fontSize, this.fontFamily].join(' ') } 
				} 
			}
			...
			this.setContext = function(ctx) {
				...
				if (typeof(ctx.font) != 'undefined') {
					ctx.font = svg.Font.CreateFont( 
						...
						this.style('font-family').value).__toString();
				}
